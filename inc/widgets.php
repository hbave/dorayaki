...
class dorayaki_team extends WP_Widget {
	...
	public function widget($args, $instance) {
					...
					<div class="tm-author">
						...
						<span class="tm-pos"><?php echo esc_html($teamposition); ?></span>
							...
							<?php if($teamlinkurl != ''){
								...
								echo '<a href="'.esc_url($teamlinkurl, $allowed_protocols).'" class="tm-link" aria-label="'.esc_attr($aria1).'">'.esc_html($teamlink).'</a>';
							} ?>
							<?php if($teamlinkurl2 != ''){
			                    $aria2 = !empty($teamlink_aria2) ? $teamlink_aria2 : sprintf( __( '%1$s %2$s', 'dorayaki' ), $teamname, $teamlink2 );
								echo '<a href="'.esc_url($teamlinkurl2, $allowed_protocols).'" class="tm-link" aria-label="'.esc_attr($aria2).'">'.esc_html($teamlink2).'</a>';
							} ?>
					</div>
				</div><!-- end .tm-info -->
				<div class="tm-quote">
					<?php 
					if ( $teamquote != '' ) {
						echo wpautop(wp_kses_post($teamquote)); 
					} else {
						echo '<p>&nbsp;</p>'; 
					}
					?>
	 ...
	 public function update($new_instance, $old_instance) {
		...
		$instance['teamlinkurl']    = esc_url_raw($new_instance['teamlinkurl'], $allowed_protocols);
		...
		$instance['teamlinkurl2']   = esc_url_raw($new_instance['teamlinkurl2'], $allowed_protocols);
	 ...
	 public function form($instance) {
		...
		<p>
						...
						<input type="text" name="<?php echo $this->get_field_name('teamname'); ?>" value="<?php echo esc_attr($teamname); ?>" class="widefat" id="<?php echo $this->get_field_id('teamname'); ?>" />
				</p>

				<p>
						<label for="<?php echo $this->get_field_id('teamimg'); ?>"><?php _e('Image URL (240x240px):','dorayaki'); ?></label>
						<input type="text" name="<?php echo $this->get_field_name('teamimg'); ?>" value="<?php echo esc_url($teamimg); ?>" class="widefat" id="<?php echo $this->get_field_id('teamimg'); ?>" />
				</p>
			...
			<textarea name="<?php echo $this->get_field_name('teamtext'); ?>" class="widefat" rows="8" id="<?php echo $this->get_field_id('teamtext'); ?>"><?php echo esc_textarea($teamtext); ?></textarea>
				</p>

				 <p>
					<label for="<?php echo $this->get_field_id('teamquote'); ?>"><?php _e('Personal Quote:','dorayaki'); ?></label>
			<textarea name="<?php echo $this->get_field_name('teamquote'); ?>" class="widefat" rows="4" id="<?php echo $this->get_field_id('teamquote'); ?>"><?php echo esc_textarea($teamquote); ?></textarea>
				</p>
					 <hr/>
				<p>
						<label for="<?php echo $this->get_field_id('teamlink'); ?>"><?php _e('1. Link (Text):','dorayaki'); ?></label>
						<input type="text" name="<?php echo $this->get_field_name('teamlink'); ?>" value="<?php echo esc_attr($teamlink); ?>" class="widefat" id="<?php echo $this->get_field_id('teamlink'); ?>" />
				</p>

				 <p>
						<label for="<?php echo $this->get_field_id('teamlinkurl'); ?>"><?php _e('1. Link (URL):','dorayaki'); ?></label>
						<input type="text" name="<?php echo $this->get_field_name('teamlinkurl'); ?>" value="<?php echo esc_url($teamlinkurl); ?>" class="widefat" id="<?php echo $this->get_field_id('teamlinkurl'); ?>" placeholder="https://..., mailto:... oder tel:..." />
				</p>
		         <p>
					    <label for="<?php echo $this->get_field_id('teamlink_aria'); ?>"><?php _e('1. Link (Aria-Label):','dorayaki'); ?></label>
					    <input type="text" name="<?php echo $this->get_field_name('teamlink_aria'); ?>" value="<?php echo esc_attr($teamlink_aria); ?>" class="widefat" id="<?php echo $this->get_field_id('teamlink_aria'); ?>" placeholder="e.g. LinkedIn Profile of John Doe" />
				</p>

				 <p>
						<label for="<?php echo $this->get_field_id('teamlink2'); ?>"><?php _e('2.Link (Text):','dorayaki'); ?></label>
						<input type="text" name="<?php echo $this->get_field_name('teamlink2'); ?>" value="<?php echo esc_attr($teamlink2); ?>" class="widefat" id="<?php echo $this->get_field_id('teamlink2'); ?>" />
				</p>

				 <p>
						<label for="<?php echo $this->get_field_id('teamlinkurl2'); ?>"><?php _e('2. Link (URL):','dorayaki'); ?></label>
						<input type="text" name="<?php echo $this->get_field_name('teamlinkurl2'); ?>" value="<?php echo esc_url($teamlinkurl2); ?>" class="widefat" id="<?php echo $this->get_field_id('teamlinkurl2'); ?>" placeholder="https://..., mailto:... oder tel:..." />
				</p>
                 <p>
				        <label for="<?php echo $this->get_field_id('teamlink_aria2'); ?>"><?php _e('2. Link (Aria-Label):','dorayaki'); ?></label>
					    <input type="text" name="<?php echo $this->get_field_name('teamlink_aria2'); ?>" value="<?php echo esc_attr($teamlink_aria2); ?>" class="widefat" id="<?php echo $this->get_field_id('teamlink_aria2'); ?>" placeholder="e.g. Twitter Account of John Doe" />
				</p>
	...
	public function widget($args, $instance) {
		...
		echo $args['before_widget']; ?>
			...
			<div class="contact-box clearfix">
				<div class="cb-map">
					<?php if($cbmapurl != ''){
						echo '<iframe src="'.esc_url($cbmapurl).'&amp;output=embed" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>';
					} ?>
				</div><!-- end .cb-map -->
				<div class="cb-info clearfix">
					...
					<div class="cb-emails">
						...
						<?php if($cbemailtitle02 != '' || $cbemail02 != ''){
							echo '<h6>'.esc_html($cbemailtitle02).'</h6><span>'.esc_html($cbemail02).'</span>';
						} ?>
						<?php if($cbemailtitle03 != '' || $cbemail03 != ''){
							echo '<h6>'.esc_html($cbemailtitle03).'</h6><span>'.esc_html($cbemail03).'</span>';
						} ?>
					</div><!-- end .cb-emails -->
				</div><!-- end .cb-info -->
			</div><!-- end .contact-box -->
	 ...
	 public function form($instance) {
		...
		<p>
			...
			<textarea name="<?php echo $this->get_field_name('cbmapurl'); ?>" class="widefat" rows="8" id="<?php echo $this->get_field_id('cbmapurl'); ?>"><?php echo esc_url($cbmapurl); ?></textarea>
				</p>
